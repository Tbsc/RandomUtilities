package tbsc.randomutils.event;
...
import tbsc.randomutils.item.ItemMysticalFood;
import tbsc.randomutils.misc.CaseHelper;
import tbsc.randomutils.misc.FoodType;
                ...
                if (isOre) { // using boolean, check if it is an ore
                    ...
                    for (Slot slot : (ArrayList<Slot>) event.player.inventoryContainer.inventorySlots) { // loop through inv slots
                        ...
                        if (slot.getStack().getItem() == pickedUp.getItem()) { // if item is the ore picked up
                            slot.decrStackSize(pickedUp.stackSize); // then decrease 1, as we don't want infinite ingots
                        }
                    }
    ...
    public void onCraft(PlayerEvent.ItemCraftedEvent event) {
        if (event.crafting != null) {
            if (event.crafting.getItem() != null) {
                ...
                stack.addEnchantment(MiscInit.enchantmentTasty, 1);
            }
        }
    }
